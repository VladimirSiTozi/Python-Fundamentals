def bring_command(command):
    while True:
        ...
        if command_given == "Shoot":
            ...
            if index <= len(sequence_of_targets) and index - 2 <= len(sequence_of_targets) and \
                    index + 2 <= len(sequence_of_targets):
                sequence_of_targets.remove(sequence_of_targets[index] and sequence_of_targets[index - power]
                                           and sequence_of_targets [index + power])
